...
def tone_pulse_action_dim(max_tones: int, max_pulses: Optional[int] = None) -> int:
    ...
    Pulse phase, length, and power sequences are generated by backbone
    recurrent decoders rather than per-pulse flat action columns.  The flat
    action row stores one Gaussian controller parameter for each recurrent
    pulse decoder (phase, length, power). ActorCritic excludes those controller
    columns from flat-action log-probability/entropy and uses the recurrent
    decoders' per-pulse methods instead, so the AR process is counted exactly
    once without scaling the action vector width with ``max_pulses``.

    Layout: five leading global controls, then four ``max_tones``-wide tone
    vectors, then seven trailing pulse/channel controls::

        [noise_color, fading_mode, rf_center_delta_hz, carrier_hz_norm,
         num_tones,
         tone_freq_mean_norms[max_tones], tone_freq_std_norms[max_tones],
         tone_amp_raw[max_tones], tone_phase_rel_rad[max_tones],
         tone_phase_offset_rad, pulse_phase_ar_control,
         pulse_length_ar_control, pulse_power_ar_control,
         pulse_phase_offset_rad, pulse_count, start_offset]

    Thus the ``12`` fixed scalar dimensions are the five leading controls plus
    the seven trailing controls; they are not all contiguous at the start of the
    row because the four tone vectors sit between them.
    """

    return 12 + (4 * int(max_tones))
...
class TonePulseTXControlNetVarLen(nn.Module):
    ...
    def __init__(
        ...
        pulse_length_ar_hidden: int = 64,
        pulse_power_ar_hidden: int = 64,
    ):
        """
        Args:
            ...
            pulse_length_ar_hidden: Hidden width for the independent LSTM pulse-length decoder.
            pulse_power_ar_hidden: Hidden width for the independent LSTM pulse-power decoder.
        """
        super().__init__()
        ...
        self.pulse_length_ar_hidden = max(8, int(pulse_length_ar_hidden))
        self.pulse_power_ar_hidden = max(8, int(pulse_power_ar_hidden))
        self.n_windows = 1
        ...
        # Independent autoregressive Gaussian decoders for contiguous pulse
        # lengths and per-pulse power. Neither recurrent state consumes or
        # predicts features from the other process.
        self.pulse_length_ar_init = nn.Linear(96, 2 * self.pulse_length_ar_hidden)
        self.pulse_length_ar_step = nn.LSTMCell(3, self.pulse_length_ar_hidden)
        self.pulse_length_log_head = nn.Linear(self.pulse_length_ar_hidden, 1)
        self.pulse_length_log_scale_head = nn.Linear(self.pulse_length_ar_hidden, 1)
        self.pulse_power_ar_init = nn.Linear(96, 2 * self.pulse_power_ar_hidden)
        self.pulse_power_ar_step = nn.LSTMCell(3, self.pulse_power_ar_hidden)
        self.pulse_power_logit_head = nn.Linear(self.pulse_power_ar_hidden, 1)
        self.pulse_power_logit_scale_head = nn.Linear(self.pulse_power_ar_hidden, 1)
        # Compact controls govern the recurrent pulse decoders. They remain in
        # the action row for decoding compatibility, but ActorCritic excludes
        # them from flat Gaussian log-probability and entropy calculations.
        self.pulse_phase_ar_control_head = nn.Linear(96, 1)
        self.pulse_length_ar_control_head = nn.Linear(96, 1)
        self.pulse_power_ar_control_head = nn.Linear(96, 1)
    ...
    def pulse_length_autoregressive(
        self,
        z: torch.Tensor,
        teacher_length_logs: Optional[torch.Tensor] = None,
        deterministic: bool = True,
    ) -> Dict[str, torch.Tensor]:
        """Unroll the independent LSTM autoregressive log-length process."""

        batch_size = int(z.shape[0])
        device = z.device
        hidden_init, cell_init = torch.chunk(self.pulse_length_ar_init(z), 2, dim=-1)
        hidden, cell = torch.tanh(hidden_init), torch.tanh(cell_init)
        min_log = math.log(float(MIN_PULSE_LENGTH_SAMPLES))
        max_log = math.log(float(MAX_PULSE_LENGTH_SAMPLES))
        log_span = max(max_log - min_log, 1.0)
        prev_value = torch.full((batch_size,), min_log, dtype=torch.float32, device=device)
        teacher = self._prepare_ar_teacher(teacher_length_logs, z, "teacher_length_logs")
        ...
        for step in range(int(self.max_pulses)):
            step_feat = self._pulse_index_features(step, batch_size, device)
            normalized = ((prev_value - min_log) / log_span).clamp(0.0, 1.0).unsqueeze(-1)
            hidden, cell = self.pulse_length_ar_step(torch.cat([normalized, step_feat], dim=-1), (hidden, cell))
            mean = min_log + log_span * torch.sigmoid(self.pulse_length_log_head(hidden).squeeze(-1))
            scale = F.softplus(self.pulse_length_log_scale_head(hidden).squeeze(-1)) + 1.0e-3
            if teacher is not None and step < teacher.shape[1]:
                current = teacher[:, step].clamp(min_log, max_log)
            elif deterministic:
                current = mean
            else:
                current = (mean + scale * torch.randn_like(mean)).clamp(min_log, max_log)
            values.append(current)
            means.append(mean)
            scales.append(scale)
            prev_value = current

        pulse_length_log = torch.stack(values, dim=1)
        return {
            "pulse_length_log": pulse_length_log,
            "pulse_length_samples_cont": torch.exp(pulse_length_log).clamp(
                float(MIN_PULSE_LENGTH_SAMPLES), float(MAX_PULSE_LENGTH_SAMPLES)
            ),
            "pulse_length_log_mean": torch.stack(means, dim=1),
            "pulse_length_log_scale": torch.stack(scales, dim=1),
        }

    def pulse_power_autoregressive(
        self,
        z: torch.Tensor,
        teacher_power_logits: Optional[torch.Tensor] = None,
        deterministic: bool = True,
    ) -> Dict[str, torch.Tensor]:
        """Unroll the independent LSTM autoregressive pulse-power process."""

        batch_size = int(z.shape[0])
        device = z.device
        hidden_init, cell_init = torch.chunk(self.pulse_power_ar_init(z), 2, dim=-1)
        hidden, cell = torch.tanh(hidden_init), torch.tanh(cell_init)
        prev_value = torch.zeros(batch_size, dtype=torch.float32, device=device)
        teacher = self._prepare_ar_teacher(teacher_power_logits, z, "teacher_power_logits")
        values: List[torch.Tensor] = []
        means: List[torch.Tensor] = []
        scales: List[torch.Tensor] = []
        ...
        for step in range(int(self.max_pulses)):
            step_feat = self._pulse_index_features(step, batch_size, device)
            hidden, cell = self.pulse_power_ar_step(
                torch.cat([torch.tanh(prev_value).unsqueeze(-1), step_feat], dim=-1), (hidden, cell)
            )
            mean = self.pulse_power_logit_head(hidden).squeeze(-1)
            scale = F.softplus(self.pulse_power_logit_scale_head(hidden).squeeze(-1)) + 1.0e-3
            if teacher is not None and step < teacher.shape[1]:
                current = teacher[:, step].clamp(-20.0, 20.0)
            elif deterministic:
                current = mean
            else:
                current = (mean + scale * torch.randn_like(mean)).clamp(-20.0, 20.0)
            values.append(current)
            means.append(mean)
            scales.append(scale)
            prev_value = current

        pulse_power_logit = torch.stack(values, dim=1)
        return {
            "pulse_power_logit": pulse_power_logit,
            "pulse_power_amp": torch.sigmoid(pulse_power_logit),
            "pulse_power_logit_mean": torch.stack(means, dim=1),
            "pulse_power_logit_scale": torch.stack(scales, dim=1),
        }

    @staticmethod
    def _prepare_ar_teacher(
        teacher: Optional[torch.Tensor], z: torch.Tensor, name: str
    ) -> Optional[torch.Tensor]:
        if teacher is None:
            return None
        prepared = teacher.to(device=z.device, dtype=torch.float32)
        if prepared.ndim == 1:
            prepared = prepared.unsqueeze(0)
        if prepared.shape[0] != z.shape[0]:
            raise ValueError(f"{name} batch size must match z")
        return prepared

    def pulse_length_autoregressive_log_prob(
        self, z: torch.Tensor, length_logs: torch.Tensor
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """Teacher-forced log probability for the independent length LSTM."""
        values = self._prepare_ar_teacher(length_logs, z, "length_logs")
        assert values is not None
        decoded = self.pulse_length_autoregressive(z, teacher_length_logs=values, deterministic=True)
        n = min(int(self.max_pulses), int(values.shape[1]))
        dist = torch.distributions.Normal(decoded["pulse_length_log_mean"][:, :n], decoded["pulse_length_log_scale"][:, :n])
        return dist.log_prob(values[:, :n]).sum(dim=-1), decoded

    def pulse_power_autoregressive_log_prob(
        self, z: torch.Tensor, power_logits: torch.Tensor
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """Teacher-forced log probability for the independent power LSTM."""
        values = self._prepare_ar_teacher(power_logits, z, "power_logits")
        assert values is not None
        decoded = self.pulse_power_autoregressive(z, teacher_power_logits=values, deterministic=True)
        n = min(int(self.max_pulses), int(values.shape[1]))
        dist = torch.distributions.Normal(decoded["pulse_power_logit_mean"][:, :n], decoded["pulse_power_logit_scale"][:, :n])
        return dist.log_prob(values[:, :n]).sum(dim=-1), decoded

    def pulse_length_autoregressive_entropy(self, z: torch.Tensor) -> torch.Tensor:
        """Entropy of the independent autoregressive pulse-length process."""
        decoded = self.pulse_length_autoregressive(z, deterministic=True)
        return torch.distributions.Normal(
            torch.zeros_like(decoded["pulse_length_log_scale"]), decoded["pulse_length_log_scale"]
        ).entropy().sum(dim=-1)

    def pulse_power_autoregressive_entropy(self, z: torch.Tensor) -> torch.Tensor:
        """Entropy of the independent autoregressive pulse-power process."""
        decoded = self.pulse_power_autoregressive(z, deterministic=True)
        return torch.distributions.Normal(
            torch.zeros_like(decoded["pulse_power_logit_scale"]), decoded["pulse_power_logit_scale"]
        ).entropy().sum(dim=-1)
    ...
    def model_outputs_from_fused(self, z: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        pulse_length = self.pulse_length_autoregressive(z, deterministic=True)
        pulse_power = self.pulse_power_autoregressive(z, deterministic=True)
        return {
            ...
            **pulse_length,
            **pulse_power,
            "pulse_phase_ar_control": torch.pi * torch.tanh(self.pulse_phase_ar_control_head(z)),
            "pulse_length_ar_control": 2.0 * torch.tanh(self.pulse_length_ar_control_head(z)),
            "pulse_power_ar_control": 2.0 * torch.tanh(self.pulse_power_ar_control_head(z)),
            "pulse_phase_offset_rad": torch.pi * torch.tanh(self.pulse_phase_offset_head(z)),
            # Legacy scalar aliases remain for old callers; contiguous AR pulse lengths are authoritative.
            "pulse_on_samples": torch.exp(pulse_length["pulse_length_log"][:, :1]).clamp(
                float(MIN_PULSE_LENGTH_SAMPLES), float(MAX_PULSE_LENGTH_SAMPLES)
            ),
            "pulse_off_samples": torch.zeros_like(pulse_length["pulse_length_log"][:, :1]),
...
class ActorCritic(nn.Module):
    ...
    def __init__(
        ...
        self.backbone = TonePulseTXControlNetVarLen(
            ...
            pulse_length_ar_hidden=pulse_length_ar_hidden,
            pulse_power_ar_hidden=pulse_power_ar_hidden,
        )
        self.max_tones = max_tones
        self.max_pulses = max_pulses
    ...
    def _action_mean_from_model_out(self, y: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        pieces = [
            ...
            y["pulse_phase_ar_control"],
            y["pulse_length_ar_control"],
            y["pulse_power_ar_control"],
    ...
    def _recurrent_pulse_control_action_slice(self) -> slice:
        """Return flat action columns for phase/length/power AR controls.

        The action layout is built in _action_mean_from_model_out. The three
        recurrent pulse controller means remain immediately after the
        tone_phase_offset scalar for decoding compatibility, but this slice is
        masked out of flat Gaussian log-probability and entropy calculations.
        """
    ...
    def _action_distribution(self, action_mean: torch.Tensor, log_std: torch.Tensor) -> torch.distributions.Normal:
        # Normal is factorized by flat action column.  Each column's loc comes
        # from action_mean and each column's scale from the same-index log_std.
        # Flat terms for AR-control columns are removed below; the full
        # autoregressive phase/length/power processes supply those terms.
    ...
    def _autoregressive_pulse_log_prob(
        self,
        fused: torch.Tensor,
        model_out: Dict[str, torch.Tensor],
    ) -> torch.Tensor:
        """Return summed log-prob of the decoded AR pulse phase/length/power sequences."""

        phase_log_prob, _ = self.backbone.pulse_phase_autoregressive_log_prob(
            fused,
            model_out["pulse_phase_rel_rad"],
        )
        length_log_prob, _ = self.backbone.pulse_length_autoregressive_log_prob(
            fused, model_out["pulse_length_log"]
        )
        power_log_prob, _ = self.backbone.pulse_power_autoregressive_log_prob(
            fused, model_out["pulse_power_logit"]
        )
        ar_log_prob = phase_log_prob + length_log_prob + power_log_prob
        return torch.nan_to_num(ar_log_prob, nan=0.0, posinf=0.0, neginf=0.0)

    def _autoregressive_pulse_entropy(self, fused: torch.Tensor) -> torch.Tensor:
        """Return summed entropy of the AR pulse phase/length/power decoders."""

        phase_entropy = self.backbone.pulse_phase_autoregressive_entropy(fused)
        length_entropy = self.backbone.pulse_length_autoregressive_entropy(fused)
        power_entropy = self.backbone.pulse_power_autoregressive_entropy(fused)
        ar_entropy = phase_entropy + length_entropy + power_entropy
        return torch.nan_to_num(ar_entropy, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def _policy_log_prob_entropy(
        ...
        *,
        ...
        """Combine flat-action and autoregressive pulse log-prob/entropy terms."""

        # The three compact AR controller columns determine decoder behavior,
        # but are not independent actions. Exclude their Gaussian p*log(p)
        # terms and account for phases/lengths/powers only through the AR
        # sequence distributions below.
        flat_mask = torch.ones_like(action)
        flat_mask[..., self._recurrent_pulse_control_action_slice()] = 0.0
        flat_log_prob = (dist.log_prob(action) * flat_mask).sum(dim=-1)
        log_prob = flat_log_prob + self._autoregressive_pulse_log_prob(fused, model_out)
        ...
        entropy = flat_entropy + self._autoregressive_pulse_entropy(fused)
...
def _apply_recurrent_pulse_action_controls(
    *,
    pulse_lengths_samples: Sequence[int],
    pulse_power_amplitudes: Sequence[float],
    pulse_phase_rotations_rad: Sequence[float],
    phase_control: float = 0.0,
    length_control: float = 0.0,
    power_control: float = 0.0,
    desired_output_iq_len: Optional[int] = None,
) -> Tuple[List[int], List[float], List[float]]:
    """Apply compact Gaussian action controls to recurrent pulse sequences.

    These controls are intentionally sequence-level parameters, not per-pulse
    action columns.  They let the policy influence AR phase, log-length, and
    power while keeping a flat action width independent of ``max_pulses``.
    """

    lengths = [int(v) for v in pulse_lengths_samples]
    powers = [float(v) for v in pulse_power_amplitudes]
    phases = [float(v) for v in pulse_phase_rotations_rad]
    count = max(len(lengths), len(powers), len(phases), 1)
    if not lengths:
        lengths = [MIN_PULSE_LENGTH_SAMPLES] * count
    if not powers:
        powers = [1.0] * count
    if not phases:
        phases = [0.0] * count

    phase = _finite_float(phase_control, 0.0)
    length = _finite_float(length_control, 0.0)
    power = _finite_float(power_control, 0.0)

    upper = int(MAX_PULSE_LENGTH_SAMPLES)
    if desired_output_iq_len is not None and desired_output_iq_len > 0:
        upper = min(upper, max(int(MIN_PULSE_LENGTH_SAMPLES), int(desired_output_iq_len)))

    length_scale = math.exp(0.25 * math.tanh(length))
    power_scale = math.exp(0.50 * math.tanh(power))
    denom = float(max(count - 1, 1))

    out_lengths: List[int] = []
    out_powers: List[float] = []
    out_phases: List[float] = []
    for i in range(count):
        base_len = lengths[min(i, len(lengths) - 1)]
        out_lengths.append(max(int(MIN_PULSE_LENGTH_SAMPLES), min(upper, int(round(base_len * length_scale)))))
        base_power = powers[min(i, len(powers) - 1)]
        out_powers.append(max(0.0, min(1.0, base_power * power_scale)))
        base_phase = phases[min(i, len(phases) - 1)]
        frac = float(i) / denom
        out_phases.append(base_phase + phase * frac)

    phase_t = _wrap_phase_rad(torch.as_tensor(out_phases, dtype=torch.float32)).to(dtype=torch.float64)
    return out_lengths, out_powers, [float(v) for v in phase_t.tolist()]
...
def decode_tone_pulse_config(
    ...
    # Variable-length AR pulses are contiguous: one pulse immediately follows the previous one.
    ...
    pulse_lengths_samples, pulse_power_amplitudes, pulse_phase_rotations_rad = _apply_recurrent_pulse_action_controls(
        ...
        phase_control=_finite_scalar_value(
            model_out.get("pulse_phase_ar_control", torch.tensor([[0.0]])),
            default=0.0,
            min_value=-math.pi,
            max_value=math.pi,
        ),
        length_control=_finite_scalar_value(
            model_out.get("pulse_length_ar_control", torch.tensor([[0.0]])),
            default=0.0,
            min_value=-5.0,
            max_value=5.0,
        ),
        power_control=_finite_scalar_value(
            model_out.get("pulse_power_ar_control", torch.tensor([[0.0]])),
            default=0.0,
            min_value=-5.0,
            max_value=5.0,
        ),
...
def apply_tone_pulse_action_overrides(
    ...
    *,
    ...
    pulse_lengths_samples, pulse_power_amplitudes, pulse_phase_rotations_rad = _apply_recurrent_pulse_action_controls(
        ...
        pulse_power_amplitudes=pulse_power_amplitudes,
        pulse_phase_rotations_rad=pulse_phase_rotations_rad,
        phase_control=_finite_float(overrides.get("pulse_phase_ar_control", 0.0), 0.0),
        length_control=_finite_float(overrides.get("pulse_length_ar_control", 0.0), 0.0),
        power_control=_finite_float(overrides.get("pulse_power_ar_control", 0.0), 0.0),
        desired_output_iq_len=desired_output_iq_len,
    )
    pulse_length_logs = [float(math.log(float(v))) for v in pulse_lengths_samples]
    pulse_on_samples = pulse_lengths_samples[0]
