...
MIN_PULSE_LENGTH_SAMPLES = 5
MAX_PULSE_LENGTH_SAMPLES = 10_000
FREQUENCY_STFT_FEATURE_CHANNELS = 14
TIMING_STFT_FEATURE_CHANNELS = 9
...
def tone_pulse_action_dim(max_tones: int, max_pulses: Optional[int] = None) -> int:
    """Return the fixed-width flattened RL action size.

    Every pulse has independent phase, log-length, and power-logit action
    columns.  Layout::

        [noise_color, fading_mode, rf_center_delta_hz, carrier_hz_norm,
         num_tones, tone_freq_mean_norms[max_tones],
         tone_freq_std_norms[max_tones], tone_amp_raw[max_tones],
         tone_phase_rel_rad[max_tones], tone_phase_offset_rad,
         pulse_phase_rel_rad[max_pulses], pulse_length_log[max_pulses],
         pulse_power_logit[max_pulses], pulse_phase_offset_rad,
         pulse_count, start_offset]
    """

    if max_pulses is None:
        raise ValueError("max_pulses is required for independent per-pulse actions")
    return 9 + (4 * int(max_tones)) + (3 * int(max_pulses))
...
def build_first_pass_scalar_side_from_iq_batch(
    iq_batch: Union[torch.Tensor, Sequence[Union[torch.Tensor, List[complex]]]],
    sample_rate_hz: float,
    *,
...
def build_first_pass_scalar_side_from_iq_sections(
    ...
    *,
    ...
    """Compatibility wrapper that joins legacy sections into one IQ object."""

    if not iq_sections:
        raise ValueError("iq_sections must contain at least one IQ batch")
    ...
    return build_first_pass_scalar_side_from_iq_batch(torch.cat(batches, dim=1), sample_rate_hz, device=device)
...
def preprocess_iq_to_stft_feature(
    ...
    # Independent timing STFT: a five-sample non-overlapping window preserves
    # the smallest requested event scale.  It is intentionally not resized to
    # the frequency STFT because the two maps are processed by separate CNNs.
    nperseg_time = int(min(TIMING_STFT_WINDOW_SAMPLES, max(2, x_work.numel())))
    noverlap_time = max(0, nperseg_time - TIMING_STFT_HOP_SAMPLES)
    hop_time = max(1, nperseg_time - noverlap_time)
    nfft_time = max(8, nperseg_time)
    ...
    # Keep a concatenated compatibility view for caches and callers using the
    # legacy ``feature`` key.  Only this compatibility view is resized; the
    # native-resolution maps below feed the parallel model branches.
    feat_time_compat = F.interpolate(
        feat_time.unsqueeze(0), size=feat_freq.shape[-2:], mode="bilinear", align_corners=False
    ).squeeze(0)
    feat = torch.cat([feat_freq, feat_time_compat], dim=0)

    return {
        "feature": _sanitize_stft_feature(feat),
        "frequency_feature": _sanitize_stft_feature(feat_freq),
...
class ResUNetSTFTEncoder(nn.Module):
    """Residual U-Net encoder tuned for one STFT resolution branch.

    The timing branch uses a dilated bottleneck whose receptive field spans at
    least 100,000 input samples while retaining the native five-sample timing
    STFT cells.  The frequency branch uses ordinary local convolutions so it
    does not blur narrow spectral features.
    """
    ...
    def __init__(
        ...
        base_ch: int = 24,
        *,
        samples_per_time_cell: int = 1,
        max_detectable_samples: int = TARGET_MAX_DETECTABLE_SAMPLES,
    ):
        super().__init__()
        self.samples_per_time_cell = max(1, int(samples_per_time_cell))
        self.max_detectable_samples = max(self.samples_per_time_cell, int(max_detectable_samples))
        # Two stride-2 encoder stages precede the bottleneck. A 3-wide dilated
        # convolution covers 2*dilation*4 cells, plus the local encoder field.
        self.bottleneck_time_dilation = max(
            1, math.ceil(self.max_detectable_samples / (8 * self.samples_per_time_cell))
        )
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(
                ...
                base_ch * 4,
                kernel_size=3,
                padding=(1, self.bottleneck_time_dilation),
                dilation=(1, self.bottleneck_time_dilation),
                bias=False,
            ),
            nn.BatchNorm2d(base_ch * 4),
            nn.ReLU(inplace=True),
            ResidualBlock2D(base_ch * 4, base_ch * 4),
        )
    ...
    @property
    def temporal_receptive_field_samples(self) -> int:
        return 8 * self.bottleneck_time_dilation * self.samples_per_time_cell
...
class TonePulseTXControlNetVarLen(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        if int(in_ch) != FREQUENCY_STFT_FEATURE_CHANNELS + TIMING_STFT_FEATURE_CHANNELS:
            raise ValueError(
                f"in_ch must be {FREQUENCY_STFT_FEATURE_CHANNELS + TIMING_STFT_FEATURE_CHANNELS} "
                "for the frequency/timing STFT feature schema"
            )
        self.frequency_encoder = ResUNetSTFTEncoder(
            in_ch=FREQUENCY_STFT_FEATURE_CHANNELS,
            base_ch=base_ch,
            samples_per_time_cell=1,
            max_detectable_samples=1,
        )
        self.timing_encoder = ResUNetSTFTEncoder(
            in_ch=TIMING_STFT_FEATURE_CHANNELS,
            base_ch=base_ch,
            samples_per_time_cell=TIMING_STFT_HOP_SAMPLES,
            max_detectable_samples=TARGET_MAX_DETECTABLE_SAMPLES,
        )
        self.max_tones = max_tones
        self.max_pulses = max_pulses
        self.n_windows = 1

        branch_dim = self.frequency_encoder.feature_dim
        self.frequency_window_fusion = nn.Sequential(
            nn.Linear(branch_dim * self.n_windows, branch_dim * 2), nn.ReLU(inplace=True),
            nn.Linear(branch_dim * 2, branch_dim), nn.ReLU(inplace=True),
        )
        self.timing_window_fusion = nn.Sequential(
            nn.Linear(branch_dim * self.n_windows, branch_dim * 2), nn.ReLU(inplace=True),
            nn.Linear(branch_dim * 2, branch_dim), nn.ReLU(inplace=True),
        )
        self.branch_fusion = nn.Sequential(
            nn.Linear(branch_dim * 2, branch_dim * 2), nn.ReLU(inplace=True),
            nn.Linear(branch_dim * 2, branch_dim), nn.ReLU(inplace=True),
        )

        self.scalar_proj = nn.Sequential(nn.Linear(n_scalar_features, 48), nn.ReLU(inplace=True))
        fused_dim = branch_dim + 48
        ...
        # Independent heads give every pulse its own phase, duration, and power.
        self.pulse_phase_rel_head = nn.Linear(96, max_pulses)
        self.pulse_length_log_head = nn.Linear(96, max_pulses)
        self.pulse_power_logit_head = nn.Linear(96, max_pulses)
        self.pulse_phase_offset_head = nn.Linear(96, 1)
        self.pulse_count_head = nn.Linear(96, 1)
        self.start_offset_head = nn.Linear(96, 1)
        self.tone_pulse_count_heads = nn.ModuleList([nn.Linear(96, 1) for _ in range(max_tones)])
        self.tone_start_offset_heads = nn.ModuleList([nn.Linear(96, 1) for _ in range(max_tones)])
        self.tone_pulse_phase_rel_heads = nn.ModuleList([nn.Linear(96, 1) for _ in range(max_tones * max_pulses)])
        self.tone_pulse_amp_heads = nn.ModuleList([nn.Linear(96, 1) for _ in range(max_tones * max_pulses)])
    ...
    def split_stft_branches(
        ...
        """Return native branch maps, or split an older concatenated cache."""
        if isinstance(feature, dict):
            return feature["frequency_feature"], feature["timing_feature"]
        expected = FREQUENCY_STFT_FEATURE_CHANNELS + TIMING_STFT_FEATURE_CHANNELS
        if feature.shape[1] != expected:
            raise ValueError(f"Expected {expected} STFT channels, got {feature.shape[1]}")
        return torch.split(feature, [FREQUENCY_STFT_FEATURE_CHANNELS, TIMING_STFT_FEATURE_CHANNELS], dim=1)

    def encode_stft_windows(
        self, stft_feature_list: List[Union[torch.Tensor, Dict[str, torch.Tensor]]]
    ) -> torch.Tensor:
        if len(stft_feature_list) != self.n_windows:
            raise ValueError(f"Expected exactly {self.n_windows} STFT windows, got {len(stft_feature_list)}")
        pairs = [self.split_stft_branches(feature) for feature in stft_feature_list]
        freq = self.frequency_window_fusion(torch.cat([self.frequency_encoder(pair[0]) for pair in pairs], dim=-1))
        timing = self.timing_window_fusion(torch.cat([self.timing_encoder(pair[1]) for pair in pairs], dim=-1))
        return self.branch_fusion(torch.cat([freq, timing], dim=-1))

    def encode_inputs(
        self,
        stft_feature_list: List[Union[torch.Tensor, Dict[str, torch.Tensor]]],
        scalar_side: torch.Tensor,
    ) -> torch.Tensor:
        z_stft = self.encode_stft_windows(stft_feature_list)
    ...
    def model_outputs_from_fused(self, z: torch.Tensor) -> Dict[str, torch.Tensor]:
        min_length_log = math.log(float(MIN_PULSE_LENGTH_SAMPLES))
        max_length_log = math.log(float(MAX_PULSE_LENGTH_SAMPLES))
        pulse_length_log = min_length_log + (max_length_log - min_length_log) * torch.sigmoid(self.pulse_length_log_head(z))
        return {
            ...
            "pulse_phase_rel_rad": torch.pi * torch.tanh(self.pulse_phase_rel_head(z)),
            "pulse_length_log": pulse_length_log,
            "pulse_length_samples_cont": torch.exp(pulse_length_log),
            "pulse_power_logit": self.pulse_power_logit_head(z),
            "pulse_phase_offset_rad": torch.pi * torch.tanh(self.pulse_phase_offset_head(z)),
            # Legacy scalar aliases remain for old callers; per-pulse lengths are authoritative.
            "pulse_on_samples": torch.exp(pulse_length_log[:, :1]).clamp(
                float(MIN_PULSE_LENGTH_SAMPLES), float(MAX_PULSE_LENGTH_SAMPLES)
            ),
            "pulse_off_samples": torch.zeros_like(pulse_length_log[:, :1]),
            "pulse_count": 1.0 + (float(self.max_pulses) - 1.0) * torch.sigmoid(self.pulse_count_head(z)),
            "start_offset": 0.0 + 4096.0 * torch.sigmoid(self.start_offset_head(z)),
            "tone_pulse_count": torch.cat([1.0 + 32.0 * torch.sigmoid(h(z)) for h in self.tone_pulse_count_heads], dim=1),
            "tone_start_offset_samples": torch.cat([4096.0 * torch.sigmoid(h(z)) for h in self.tone_start_offset_heads], dim=1),
            "tone_pulse_phase_rel_rad": torch.cat([torch.pi * torch.tanh(h(z)) for h in self.tone_pulse_phase_rel_heads], dim=1),
            "tone_pulse_amp": torch.cat([torch.sigmoid(h(z)) for h in self.tone_pulse_amp_heads], dim=1),
    ...
    def forward(self, stft_feature_list: List[torch.Tensor], scalar_side: torch.Tensor) -> Dict[str, torch.Tensor]:
        return self.model_outputs_from_fused(self.encode_inputs(stft_feature_list, scalar_side))
...
class ActorCritic(nn.Module):
    ...
    def __init__(
        ...
        self.max_tones = max_tones
        self.max_pulses = max_pulses
        self.action_dim = int(action_dim) if action_dim is not None else tone_pulse_action_dim(max_tones, max_pulses)
        if self.action_dim <= 0:
            raise ValueError("action_dim must be positive")

        self._action_std_floor = 1e-4
        self._init_log_std = math.log(max(action_std_init, self._action_std_floor))
        # One learned log-std is emitted for each compatibility action column.
        # AR-control columns are excluded from flat probability/entropy sums.
    ...
    def _encode(
        ...
        return self.backbone.encode_inputs(stft_feature_list, scalar_side)
    ...
    def _action_mean_from_model_out(self, y: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if action_mean.shape[-1] != self.action_dim:
            raise ValueError(
                ...
                "Set action_dim to tone_pulse_action_dim(max_tones, max_pulses) or a matching custom width."
            )
        return action_mean
    ...
    @staticmethod
    def _observation_stft_reference(stft_list: Sequence[Any]) -> torch.Tensor:
        if not stft_list:
            raise ValueError("observation['stft_feature_list'] must contain one feature view")
        view = stft_list[0]
        if isinstance(view, dict):
            return view["frequency_feature"]
        return view
    ...
    def forward_observation(self, observation: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if scalar_side is None:
            ...
            ref = self._observation_stft_reference(stft_list)
    ...
    def _action_distribution(self, action_mean: torch.Tensor, log_std: torch.Tensor) -> torch.distributions.Normal:
        """Return the fully factorized distribution over independent action columns."""
    ...
    def _policy_log_prob_entropy(
        self, *, dist: torch.distributions.Normal, action: torch.Tensor, include_entropy: bool = True, **_: Any
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        log_prob = torch.nan_to_num(dist.log_prob(action).sum(dim=-1), nan=0.0, posinf=0.0, neginf=0.0)
        entropy = dist.entropy().sum(dim=-1) if include_entropy else torch.zeros_like(log_prob)
        return log_prob, torch.nan_to_num(entropy, nan=0.0, posinf=0.0, neginf=0.0)

    def _sample_hybrid_action(
        self, *, action_mean: torch.Tensor, log_std: torch.Tensor, deterministic: bool, **_: Any
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        log_prob, entropy = self._policy_log_prob_entropy(dist=dist, action=action)
    ...
    def act(
        ...
        action_mean, value, log_std, fused, model_out = self._policy_tensors(stft_feature_list=stft_feature_list, scalar_side=scalar_side)
    ...
    def evaluate_actions(
        ...
        action_mean, value, log_std, fused, model_out = self._policy_tensors(stft_feature_list=stft_feature_list, scalar_side=scalar_side)
        dist = self._action_distribution(action_mean=action_mean, log_std=log_std)
        log_prob, entropy = self._policy_log_prob_entropy(
            dist=dist,
            action=actions,
            fused=fused,
            model_out=model_out,
    ...
    def get_action_value_logp(
        ...
        action_mean, value, log_std, fused, model_out = self._policy_tensors_from_observation(observation)
        if action is None:
            action, log_prob, _ = self._sample_hybrid_action(
                action_mean=action_mean,
                log_std=log_std,
            ...
            dist = self._action_distribution(action_mean=action_mean, log_std=log_std)
            log_prob, _ = self._policy_log_prob_entropy(
                dist=dist,
                action=action,
                fused=fused,
                model_out=model_out,
...
def _pulse_length_logs_to_samples(
    length_logs: Union[torch.Tensor, Sequence[float]],
    *,
    count: int,
    desired_output_iq_len: Optional[int] = None,
) -> List[int]:
    """Convert network-emitted log pulse lengths to bounded integer samples."""

    vals = _finite_vector(
        torch.as_tensor(length_logs) if not torch.is_tensor(length_logs) else length_logs,
        default=math.log(float(MIN_PULSE_LENGTH_SAMPLES)),
        min_value=math.log(float(MIN_PULSE_LENGTH_SAMPLES)),
        max_value=math.log(float(MAX_PULSE_LENGTH_SAMPLES)),
        dtype=torch.float64,
    )
    if vals.numel() == 0:
        vals = torch.full((max(1, int(count)),), math.log(float(MIN_PULSE_LENGTH_SAMPLES)), dtype=torch.float64)
    out: List[int] = []
    upper = int(MAX_PULSE_LENGTH_SAMPLES)
    if desired_output_iq_len is not None and desired_output_iq_len > 0:
        upper = min(upper, max(int(MIN_PULSE_LENGTH_SAMPLES), int(desired_output_iq_len)))
    for i in range(max(1, int(count))):
        raw = vals[min(i, int(vals.numel()) - 1)]
        length = int(round(float(torch.exp(raw).item())))
        out.append(max(int(MIN_PULSE_LENGTH_SAMPLES), min(upper, length)))
    return out


def _pulse_power_logits_to_amplitudes(
    power_logits: Union[torch.Tensor, Sequence[float]],
    *,
    count: int,
) -> List[float]:
    """Convert network-emitted pulse power logits to finite [0, 1] amplitude multipliers."""

    vals = _finite_vector(
        torch.as_tensor(power_logits) if not torch.is_tensor(power_logits) else power_logits,
        default=0.0,
        min_value=-20.0,
        max_value=20.0,
        dtype=torch.float64,
    ...
    if vals.numel() == 0:
        vals = torch.zeros(max(1, int(count)), dtype=torch.float64)
    amps = torch.sigmoid(vals).tolist()
    return [max(0.0, min(1.0, float(amps[min(i, len(amps) - 1)]))) for i in range(max(1, int(count)))]
...
def _apply_per_tone_pulse_phase_rotation(
    ...
    *,
    ...
    tone_pulse_amplitudes: Optional[Sequence[Sequence[float]]] = None,
    tone_pulse_lengths_samples: Optional[Sequence[Sequence[int]]] = None,
) -> torch.Tensor:
    ...
    for k in range(num_tones):
        ...
        pulse_amps = tone_pulse_amplitudes[k] if tone_pulse_amplitudes is not None and k < len(tone_pulse_amplitudes) else []
        pulse_lengths = (
            tone_pulse_lengths_samples[k]
            if tone_pulse_lengths_samples is not None and k < len(tone_pulse_lengths_samples)
            else []
        )
        if count <= 0:
            continue
        ...
        for p in range(count):
            pulse_len = (
                int(pulse_lengths[min(p, len(pulse_lengths) - 1)])
                if len(pulse_lengths)
                else on
            )
            pulse_len = max(int(MIN_PULSE_LENGTH_SAMPLES), min(int(MAX_PULSE_LENGTH_SAMPLES), pulse_len))
            start = cursor
            if start >= n:
                break
            end = min(start + pulse_len, n)
            theta = float(phases[min(p, len(phases) - 1)]) if len(phases) else 0.0
            amp_step = float(pulse_amps[min(p, len(pulse_amps) - 1)]) if len(pulse_amps) else 1.0
            mask[start:end] = max(0.0, amp_step)
...
def decode_tone_pulse_config(
    ...
    pulse_length_logs_t = _finite_vector(
        model_out.get("pulse_length_log", torch.tensor([])),
        default=math.log(float(MIN_PULSE_LENGTH_SAMPLES)),
        min_value=math.log(float(MIN_PULSE_LENGTH_SAMPLES)),
        max_value=math.log(float(MAX_PULSE_LENGTH_SAMPLES)),
        dtype=torch.float64,
    )[:pulse_count]
    pulse_lengths_samples = _pulse_length_logs_to_samples(
        pulse_length_logs_t,
        count=pulse_count,
        desired_output_iq_len=desired_output_iq_len,
    )
    pulse_length_logs = [float(math.log(float(v))) for v in pulse_lengths_samples]
    pulse_power_amplitudes = _pulse_power_logits_to_amplitudes(
        model_out.get("pulse_power_logit", torch.tensor([])),
        count=pulse_count,
    )
    # Variable-length pulses are contiguous: one pulse immediately follows the previous one.
    pulse_on_samples = pulse_lengths_samples[0]
    pulse_off_samples = 0
    start_offset_samples = int(
    ...
    tone_pulse_amplitudes = []
    tone_pulse_lengths_samples = []
    tone_on_direct = _finite_vector(model_out.get("tone_pulse_on_samples", torch.tensor([])), default=float(pulse_on_samples), dtype=torch.float64)
    tone_off_direct = _finite_vector(model_out.get("tone_pulse_off_samples", torch.tensor([])), default=float(pulse_off_samples), dtype=torch.float64)
    tone_count_direct = _finite_vector(model_out.get("tone_pulse_count", torch.tensor([])), default=float(pulse_count), dtype=torch.float64)
    tone_start_direct = _finite_vector(model_out.get("tone_start_offset_samples", torch.tensor([])), default=float(start_offset_samples), dtype=torch.float64)
    tone_phase_direct = _finite_vector(model_out.get("tone_pulse_phase_rel_rad", torch.tensor([])), default=0.0, dtype=torch.float64)
    tone_amp_direct = _finite_vector(model_out.get("tone_pulse_amp", torch.tensor([])), default=1.0, dtype=torch.float64)
    for k in range(num_tones):
        on_k = int(round(float(tone_on_direct[k].item()))) if k < tone_on_direct.numel() else max(1, pulse_on_samples + int(round((k - (num_tones - 1) / 2.0) * 0.1 * pulse_on_samples)))
        off_k = int(round(float(tone_off_direct[k].item()))) if k < tone_off_direct.numel() else max(0, pulse_off_samples + int(round((k - (num_tones - 1) / 2.0) * 0.1 * max(1, pulse_off_samples))))
        count_k = int(round(float(tone_count_direct[k].item()))) if k < tone_count_direct.numel() else max(1, pulse_count - k)
        start_k = int(round(float(tone_start_direct[k].item()))) if k < tone_start_direct.numel() else max(0, start_offset_samples + (k * max(1, on_k // 8)))
        on_k = max(1, on_k)
        off_k = max(0, off_k)
        count_k = max(1, min(count_k, max_pulses))
        start_k = max(0, start_k)
        if desired_output_iq_len is not None and desired_output_iq_len > 0:
            on_k = min(on_k, desired_output_iq_len)
            start_k = min(start_k, max(0, desired_output_iq_len - 1))
        lengths_k = [int(pulse_lengths_samples[min(p, len(pulse_lengths_samples) - 1)]) for p in range(count_k)]
        on_k = lengths_k[0] if lengths_k else int(on_k)
        off_k = 0
        tone_pulse_on_samples.append(int(on_k))
        tone_pulse_off_samples.append(0)
        tone_pulse_count.append(int(count_k))
        tone_pulse_lengths_samples.append(lengths_k)
        ...
        if tone_phase_direct.numel() >= (k + 1) * max_pulses:
            phase_list_k = tone_phase_direct[k * max_pulses : k * max_pulses + count_k]
        elif count_k > 1:
        ...
        if tone_amp_direct.numel() >= (k + 1) * max_pulses:
            amps_k = tone_amp_direct[k * max_pulses : k * max_pulses + count_k].tolist()
        else:
            amps_k = [pulse_power_amplitudes[min(p, len(pulse_power_amplitudes) - 1)] for p in range(count_k)]
        tone_pulse_amplitudes.append([max(0.0, float(a)) for a in amps_k])
    ...
    return TonePulseControlConfig(
        ...
        pulse_on_samples=max(int(MIN_PULSE_LENGTH_SAMPLES), pulse_on_samples),
        pulse_off_samples=0,
        pulse_count=pulse_count,
        start_offset_samples=max(0, start_offset_samples),
        pulse_lengths_samples=pulse_lengths_samples,
        pulse_length_logs=pulse_length_logs,
        pulse_phase_rotations_rad=pulse_phase_rotations_rad,
        pulse_power_amplitudes=pulse_power_amplitudes,
        ...
        tone_pulse_lengths_samples=tone_pulse_lengths_samples,
...
def apply_tone_pulse_action_overrides(
    ...
    *,
    ...
    if "pulse_length_log" in overrides:
        pulse_lengths_samples = _pulse_length_logs_to_samples(
            overrides["pulse_length_log"],
            count=pulse_count,
            desired_output_iq_len=desired_output_iq_len,
        )
    elif "pulse_lengths_samples" in overrides:
        raw_lengths = _finite_float_list(
            overrides["pulse_lengths_samples"],
            default=cfg.pulse_lengths_samples or [cfg.pulse_on_samples],
            length=pulse_count,
        )
        pulse_lengths_samples = [
            max(int(MIN_PULSE_LENGTH_SAMPLES), min(int(MAX_PULSE_LENGTH_SAMPLES), int(round(v))))
            for v in raw_lengths
        ]
    else:
        pulse_lengths_samples = _finite_float_list(
            cfg.pulse_lengths_samples or [cfg.pulse_on_samples],
            default=[cfg.pulse_on_samples],
            length=pulse_count,
        )
        pulse_lengths_samples = [
            max(int(MIN_PULSE_LENGTH_SAMPLES), min(int(MAX_PULSE_LENGTH_SAMPLES), int(round(v))))
            for v in pulse_lengths_samples
        ]
    pulse_length_logs = [float(math.log(float(v))) for v in pulse_lengths_samples]
    pulse_power_amplitudes = (
        _pulse_power_logits_to_amplitudes(overrides["pulse_power_logit"], count=pulse_count)
        if "pulse_power_logit" in overrides
        else _finite_float_list(
            overrides.get("pulse_power_amplitudes", cfg.pulse_power_amplitudes),
            default=cfg.pulse_power_amplitudes or [1.0],
            length=pulse_count,
        )
    )
    pulse_power_amplitudes = [max(0.0, min(1.0, float(v))) for v in pulse_power_amplitudes]
    pulse_on_samples = pulse_lengths_samples[0]
    pulse_off_samples = 0
    ...
    pulse_phase_rotations_rad = _finite_float_list(
        overrides.get("pulse_phase_rel_rad", overrides.get("pulse_phase_rotations_rad", cfg.pulse_phase_rotations_rad)),
    ...
    pulse_length_logs = [float(math.log(float(v))) for v in pulse_lengths_samples]
    pulse_on_samples = pulse_lengths_samples[0]
    pulse_phase_rel_rad = pulse_phase_rotations_rad
    ...
    tone_pulse_lengths_samples = [
        [int(pulse_lengths_samples[min(p, len(pulse_lengths_samples) - 1)]) for p in range(ck)]
        for ck in tone_pulse_count
    ]
    tone_pulse_on_samples = [lengths[0] if lengths else int(MIN_PULSE_LENGTH_SAMPLES) for lengths in tone_pulse_lengths_samples]
    tone_pulse_off_samples = [0 for _ in tone_pulse_count]
    tone_pulse_amplitudes = []
    override_tone_pulse_amplitudes = overrides.get("tone_pulse_amplitudes")
    for k in range(num_tones):
        ck = tone_pulse_count[k] if k < len(tone_pulse_count) else pulse_count
        base = float(tone_amplitudes[k]) if k < len(tone_amplitudes) else 1.0
        if isinstance(override_tone_pulse_amplitudes, (list, tuple)) and k < len(override_tone_pulse_amplitudes):
            tone_pulse_amplitudes.append(
                [max(0.0, float(v)) for v in _finite_float_list(override_tone_pulse_amplitudes[k], default=[base], length=ck)]
            )
        else:
            tone_pulse_amplitudes.append(
                [pulse_power_amplitudes[min(p, len(pulse_power_amplitudes) - 1)] for p in range(ck)]
            )
...
def build_controlled_tone_pulse_from_variable_inputs(
    ...
    """Build one controlled tone-pulse transmission from one complete RX IQ object."""
    if len(rx_iq_windows) != 1:
        raise ValueError("rx_iq_windows must contain one complete IQ object")
    batched = [_as_batch_complex_tensor([rx_iq_windows[0]])]
...
def build_controlled_tone_pulse_batch_from_iq_batches(
    ...
    if torch.is_tensor(rx_iq_batches):
        rx_iq_batches = [rx_iq_batches]
    if len(rx_iq_batches) != 1:
        raise ValueError(f"rx_iq_batches must contain one complete IQ batch, got {len(rx_iq_batches)}")

    proc = [preprocess_batched_iq_to_stft_feature(rx_iq_batches[0], intake_sample_rate_hz)]
    batch_size = int(proc[0]["feature"].shape[0])
    if any(int(p["feature"].shape[0]) != batch_size for p in proc):
        raise ValueError("IQ input batches must use the same batch size")
    ...
    use_native_stft_branches = hasattr(model, "frequency_encoder") and hasattr(model, "timing_encoder")
    if use_native_stft_branches:
        stft_tensors = [
            {
                "frequency_feature": p["frequency_feature"].to(device),
                "timing_feature": p["timing_feature"].to(device),
            }
            for p in proc
        ]
    else:
        stft_tensors = [p["feature"].to(device) for p in proc]
    expected_channels = None if use_native_stft_branches else _expected_stft_channels(model)
    ...
    for i in range(batch_size):
        ...
        if target_num_samples is None:
            target_num_samples = int(cfg.start_offset_samples) + int(sum(cfg.pulse_lengths_samples))
        ...
        tx_metadata["controller_pulse_lengths_samples"] = [int(v) for v in cfg.pulse_lengths_samples]
        tx_metadata["controller_pulse_length_logs"] = [float(v) for v in cfg.pulse_length_logs]
        tx_metadata["controller_pulse_phase_rotations_rad"] = [float(v) for v in cfg.pulse_phase_rotations_rad]
        tx_metadata["controller_pulse_power_amplitudes"] = [float(v) for v in cfg.pulse_power_amplitudes]
        ...
        tx_metadata["controller_tone_pulse_lengths_samples"] = [
            [int(length) for length in lengths] for lengths in cfg.tone_pulse_lengths_samples
        ]
        ...
        tx_metadata["controller_tone_pulse_amplitudes"] = [
            [float(a) for a in alist] for alist in cfg.tone_pulse_amplitudes
        ]
