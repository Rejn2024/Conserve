...
def tone_pulse_action_dim(max_tones: int, max_pulses: Optional[int] = None) -> int:
    ...
    pulse decoder (phase, length, power).  ActorCritic combines the flat action
    log-prob/entropy with the recurrent decoders' per-pulse log-prob/entropy,
    so the full AR pulse process is accounted for without scaling the action
    vector width with ``max_pulses``.
...
class TonePulseTXControlNetVarLen(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            pulse_length_ar_hidden: Hidden width for the LSTM pulse length/power decoder.
        """
        super().__init__()
        ...
        # Autoregressive Gaussian heads for contiguous pulse lengths and per-pulse power.
        # The network emits log pulse lengths; decode clamps exp(log_length) to [5, 10000].
        self.pulse_length_ar_init = nn.Linear(96, 2 * self.pulse_length_ar_hidden)
        self.pulse_length_ar_step = nn.LSTMCell(4, self.pulse_length_ar_hidden)
        ...
        self.pulse_power_logit_head = nn.Linear(self.pulse_length_ar_hidden, 1)
        self.pulse_power_logit_scale_head = nn.Linear(self.pulse_length_ar_hidden, 1)
        # Compact Gaussian action means that govern the recurrent pulse decoders.
        # ActorCritic samples these three parameters with learned stds, so pulse
        # phase/length/power controller variation contributes to action log-prob
        # and entropy without adding per-pulse columns to the flat action row.
    ...
    def pulse_length_power_autoregressive(
        self,
        z: torch.Tensor,
        ...
        """Unroll an LSTM autoregressive process for log pulse length and power.

        Each step conditions on the previous generated log length and power
        logit, plus the same normalized pulse-index features used by the phase
        AR decoder.  The emitted ``pulse_length_log`` is intentionally in log
        samples; synthesis converts it with ``exp`` and clamps to
        [MIN_PULSE_LENGTH_SAMPLES, MAX_PULSE_LENGTH_SAMPLES].
        """

        batch_size = int(z.shape[0])
        device = z.device
        init_state = self.pulse_length_ar_init(z)
        hidden_init, cell_init = torch.chunk(init_state, 2, dim=-1)
        hidden = torch.tanh(hidden_init)
        cell = torch.tanh(cell_init)
        prev_log_length = torch.full(
            (batch_size,),
            math.log(float(MIN_PULSE_LENGTH_SAMPLES)),
            dtype=torch.float32,
            device=device,
        )
        prev_power_logit = torch.zeros(batch_size, dtype=torch.float32, device=device)

        teacher_lengths = None
        if teacher_length_logs is not None:
            teacher_lengths = teacher_length_logs.to(device=device, dtype=torch.float32)
            if teacher_lengths.ndim == 1:
                teacher_lengths = teacher_lengths.unsqueeze(0)
            if teacher_lengths.shape[0] != batch_size:
                raise ValueError("teacher_length_logs batch size must match z")

        teacher_powers = None
        if teacher_power_logits is not None:
            teacher_powers = teacher_power_logits.to(device=device, dtype=torch.float32)
            if teacher_powers.ndim == 1:
                teacher_powers = teacher_powers.unsqueeze(0)
            if teacher_powers.shape[0] != batch_size:
                raise ValueError("teacher_power_logits batch size must match z")

        log_length_steps: List[torch.Tensor] = []
        log_length_mean_steps: List[torch.Tensor] = []
        log_length_scale_steps: List[torch.Tensor] = []
        power_logit_steps: List[torch.Tensor] = []
        power_logit_mean_steps: List[torch.Tensor] = []
        power_logit_scale_steps: List[torch.Tensor] = []
        ...
        for step in range(int(self.max_pulses)):
            step_feat = self._pulse_index_features(step, batch_size, device)
            length_norm = (prev_log_length - min_log) / log_span
            ar_in = torch.cat(
                [
                    length_norm.clamp(0.0, 1.0).unsqueeze(-1),
                    torch.tanh(prev_power_logit).unsqueeze(-1),
                    step_feat,
                ],
                dim=-1,
            )
            hidden, cell = self.pulse_length_ar_step(ar_in, (hidden, cell))
            log_length_mean = min_log + log_span * torch.sigmoid(self.pulse_length_log_head(hidden).squeeze(-1))
            log_length_scale = F.softplus(self.pulse_length_log_scale_head(hidden).squeeze(-1)) + 1.0e-3
            power_logit_mean = self.pulse_power_logit_head(hidden).squeeze(-1)
            power_logit_scale = F.softplus(self.pulse_power_logit_scale_head(hidden).squeeze(-1)) + 1.0e-3

            if teacher_lengths is not None and step < teacher_lengths.shape[1]:
                current_log_length = teacher_lengths[:, step].clamp(min_log, max_log)
            elif deterministic:
                current_log_length = log_length_mean
            else:
                current_log_length = (log_length_mean + log_length_scale * torch.randn_like(log_length_mean)).clamp(min_log, max_log)

            if teacher_powers is not None and step < teacher_powers.shape[1]:
                current_power_logit = teacher_powers[:, step].clamp(-20.0, 20.0)
            elif deterministic:
                current_power_logit = power_logit_mean
            else:
                current_power_logit = (power_logit_mean + power_logit_scale * torch.randn_like(power_logit_mean)).clamp(-20.0, 20.0)

            log_length_steps.append(current_log_length)
            log_length_mean_steps.append(log_length_mean)
            log_length_scale_steps.append(log_length_scale)
            power_logit_steps.append(current_power_logit)
            power_logit_mean_steps.append(power_logit_mean)
            power_logit_scale_steps.append(power_logit_scale)
            prev_log_length = current_log_length
            prev_power_logit = current_power_logit

        pulse_length_log = torch.stack(log_length_steps, dim=1)
        pulse_power_logit = torch.stack(power_logit_steps, dim=1)
        return {
            ...
            "pulse_length_log_mean": torch.stack(log_length_mean_steps, dim=1),
            "pulse_length_log_scale": torch.stack(log_length_scale_steps, dim=1),
            "pulse_power_logit": pulse_power_logit,
            "pulse_power_amp": torch.sigmoid(pulse_power_logit),
            "pulse_power_logit_mean": torch.stack(power_logit_mean_steps, dim=1),
            "pulse_power_logit_scale": torch.stack(power_logit_scale_steps, dim=1),
        }

    def pulse_length_power_autoregressive_log_prob(
        self,
        z: torch.Tensor,
        length_logs: torch.Tensor,
        power_logits: torch.Tensor,
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """Teacher-forced log probability for pulse log-length and power AR controls."""

        length_logs = length_logs.to(device=z.device, dtype=torch.float32)
        power_logits = power_logits.to(device=z.device, dtype=torch.float32)
        if length_logs.ndim == 1:
            length_logs = length_logs.unsqueeze(0)
        if power_logits.ndim == 1:
            power_logits = power_logits.unsqueeze(0)
        mix = self.pulse_length_power_autoregressive(
            z,
            teacher_length_logs=length_logs,
            teacher_power_logits=power_logits,
            deterministic=True,
        )
        n = min(int(self.max_pulses), int(length_logs.shape[1]), int(power_logits.shape[1]))
        length_dist = torch.distributions.Normal(
            mix["pulse_length_log_mean"][:, :n], mix["pulse_length_log_scale"][:, :n]
        )
        power_dist = torch.distributions.Normal(
            mix["pulse_power_logit_mean"][:, :n], mix["pulse_power_logit_scale"][:, :n]
        )
        per_step = length_dist.log_prob(length_logs[:, :n]).sum(dim=-1) + power_dist.log_prob(power_logits[:, :n]).sum(dim=-1)
        return per_step, mix

    def pulse_length_power_autoregressive_entropy(self, z: torch.Tensor) -> torch.Tensor:
        """Entropy of the Gaussian AR pulse length/power process under deterministic feedback."""

        mix = self.pulse_length_power_autoregressive(z, deterministic=True)
        length_entropy = torch.distributions.Normal(
            torch.zeros_like(mix["pulse_length_log_scale"]), mix["pulse_length_log_scale"]
        ).entropy()
        power_entropy = torch.distributions.Normal(
            torch.zeros_like(mix["pulse_power_logit_scale"]), mix["pulse_power_logit_scale"]
        ).entropy()
        return (length_entropy + power_entropy).sum(dim=-1)
    ...
    def model_outputs_from_fused(self, z: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        pulse_length_power = self.pulse_length_power_autoregressive(z, deterministic=True)
        return {
            ...
            **pulse_length_power,
            ...
            "pulse_on_samples": torch.exp(pulse_length_power["pulse_length_log"][:, :1]).clamp(
                float(MIN_PULSE_LENGTH_SAMPLES), float(MAX_PULSE_LENGTH_SAMPLES)
            ),
            "pulse_off_samples": torch.zeros_like(pulse_length_power["pulse_length_log"][:, :1]),
...
class ActorCritic(nn.Module):
    ...
    def __init__(
        ...
        # One learned log-std is emitted for each flat action_mean column.
        # Because action_mean includes pulse_phase_ar_control,
        # pulse_length_ar_control, and pulse_power_ar_control, the matching
        # log_std columns at _recurrent_pulse_control_action_slice() are their
        # Gaussian log standard deviations.
    ...
    def _recurrent_pulse_control_action_slice(self) -> slice:
        ...
        recurrent pulse controller means are placed immediately after the
        tone_phase_offset scalar, so log_std_head supplies their Gaussian stds
    ...
    def _action_distribution(self, action_mean: torch.Tensor, log_std: torch.Tensor) -> torch.distributions.Normal:
        ...
        # This flat term is combined with recurrent pulse-sequence terms below
        # so action log-prob/entropy covers both compact controls and the full
        # autoregressive pulse phase/length/power processes.
    ...
    def _autoregressive_pulse_log_prob(
        ...
        length_power_log_prob, _ = self.backbone.pulse_length_power_autoregressive_log_prob(
            fused,
            model_out["pulse_length_log"],
            model_out["pulse_power_logit"],
        )
        ar_log_prob = phase_log_prob + length_power_log_prob
    ...
    def _autoregressive_pulse_entropy(self, fused: torch.Tensor) -> torch.Tensor:
        ...
        length_power_entropy = self.backbone.pulse_length_power_autoregressive_entropy(fused)
        ar_entropy = phase_entropy + length_power_entropy
    ...
    def _policy_log_prob_entropy(
        ...
        *,
        ...
        flat_log_prob = dist.log_prob(action).sum(dim=-1)
