...
def tone_pulse_action_dim(max_tones: int, max_pulses: Optional[int] = None) -> int:
    ...
    pulse decoder (phase, length, power), so policy log-prob/entropy includes
    those pulse-control phenomena without scaling with ``max_pulses``.
...
class ActorCritic(nn.Module):
    ...
    def _action_distribution(self, action_mean: torch.Tensor, log_std: torch.Tensor) -> torch.distributions.Normal:
        ...
        # This includes the three recurrent pulse-controller columns, so their
        # sampled values contribute to log_prob and entropy exactly like other
        # Gaussian controls.
    ...
    def evaluate_actions(
        ...
        action_mean, value, log_std, _, _ = self._policy_tensors(stft_feature_list=stft_feature_list, scalar_side=scalar_side)
        dist = self._action_distribution(action_mean=action_mean, log_std=log_std)
