...
--------
TX:
    python advanced_link_skdsp_v3.py tx \
        --message "hello world" \
        --output tx.iq \
...
RX:
    python advanced_link_skdsp_v3.py rx \
        --input tx.iq \
        --rf-center-hz 2400100000 \
        --fec conv \
        --interleave
"""
...
SYNC_WORD = b"\xD3\x91\xC5\x7A"   # 32 bits
...
#   magic:        2 bytes
#   payload_len:  4 bytes
#   payload_len_inv: 4 bytes
#   header_crc32 over previous 10 bytes: 4 bytes
HEADER_BYTES_LEN = 2 + 4 + 4 + 4
HEADER_BITS_LEN = HEADER_BYTES_LEN * 8
HEADER_PROT_BITS_LEN = HEADER_BITS_LEN * 3   # repetition-3 protected


# -----------------------------------------------------------------------------
# Pulse shaping / power helpers
# -----------------------------------------------------------------------------

def rrc_taps(beta: float, sps: int, span: int) -> np.ndarray:
    """Root-raised-cosine FIR, normalized to unit energy."""
...
def apply_frequency_offset(iq: np.ndarray, freq_offset: float) -> np.ndarray:
    """
    Residual frequency offset in cycles/sample.
    """
    if freq_offset == 0.0:
...
def apply_carrier_frequency(iq: np.ndarray, carrier_hz: float, sample_rate_hz: float) -> np.ndarray:
    """
    Shift complex IQ within sampled bandwidth by carrier_hz.
    """
    if sample_rate_hz <= 0:
...
def apply_timing_offset_resample(iq: np.ndarray, timing_offset: float) -> np.ndarray:
    """
    Approximate sample-rate mismatch by resampling.
    """
...
# -----------------------------------------------------------------------------
# Interleaving / repetition / header helpers
# -----------------------------------------------------------------------------
...
def build_payload_bytes(message: Union[str, bytes]) -> bytes:
...
def build_tx_bitstream(
    ...
    payload_bytes = build_payload_bytes(message)
    payload_len = len(payload_bytes) - 4
    ...
    payload_bits_plain = bytes_to_bits_msb(payload_bytes)
...
# -----------------------------------------------------------------------------
# Known reference waveforms/symbols
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Acquisition, estimation, equalization
# -----------------------------------------------------------------------------
...
def coarse_frequency_acquire(
    ...
    """
    Brute-force coarse frequency acquisition:
    search CFO bins and correlate against known oversampled access waveform.
    Returns:
        start_index_samples,
        coarse_cfo_hz,
        metric
...
def extract_symbols_from_start(
    ...
    """
    Extract symbol-rate samples beginning at the matched-filter delay.
    """
...
def estimate_residual_cfo_from_preamble(
    ...
    """
    Estimate residual CFO from the two repeated halves of the preamble.

    symbols include the reference symbol at index 0.
    """
...
def design_symbol_equalizer_ls(
    ...
    """
    Symbol-spaced complex least-squares equalizer using known training symbols.
    """
    if ntaps % 2 == 0:
...
def fine_phase_correct_from_training(
    ...
    """
    Use the known training symbols to estimate residual constant phase.
    """
...
def differential_soft_demod(symbols: np.ndarray) -> np.ndarray:
    """
    Soft DBPSK differential detector.
    Positive soft => bit 1, negative soft => bit 0.
    """
    if len(symbols) < 2:
...
def tx_command(args):
    ...
    # Place signal at requested digital carrier offset within IQ bandwidth
...
def try_decode_from_symbols(
    ...
    """
    Decode a frame from already sampled symbol-rate data.
    """
    if len(symbols) < len(access_ref_symbols) + 16:
        return None

    # Residual CFO from repeated preamble halves
    ...
    # Fine constant phase correction from training
...
def rx_command(args):
    ...
    # Symbol rate implied by TX DSP
    symbol_rate_hz = tx_sample_rate_hz / sps

    # Tune to requested RX RF center
    ...
    # Internal DSP resample back to TX DSP sample rate so sps stays integer
    ...
    # Small sample-phase search around acquisition point
...
def main(argv=None):
    ...
    if args.cmd == "tx":
        if args.message is None and args.message_file is None:
            raise SystemExit("tx requires --message or --message-file")
